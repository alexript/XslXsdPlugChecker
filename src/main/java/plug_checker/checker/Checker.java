...
public class Checker {
    ...
    private String idFilePrefixCheck(){
        ...
        if(current == null) return "Parameter " + filePrefixAttribute + " not found";
        ...
        if(secondIndex < 0 || secondIndex >= xsdPathWithoutDirs.length()) return "xsd file prefix not found or it's in wrong format";
    ...
    private String cutDirsFromPath(String filePath){
        ...
        if (slashIndex > 0) result = filePath.substring(slashIndex);

        int backSlashIndex = result.lastIndexOf("\\") + 1;
        if(backSlashIndex > 0) result = result.substring(backSlashIndex);
